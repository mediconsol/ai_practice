...
export function CreateProgramDialog({ open, onOpenChange }: CreateProgramDialogProps) {
  ...
  const [programType, setProgramType] = useState<ProgramType>("chat");
  const [selectedIcon, setSelectedIcon] = useState("Sparkles");
  const [selectedGradient, setSelectedGradient] = useState("from-primary to-info");
    ...
    createProgramMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          setSelectedGradient("from-primary to-info");
          setIsPublic(true);
          setIsNew(false);
          setFormSchema(null);
          onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>새 AI 프로그램 만들기</DialogTitle>
          <DialogDescription>
            업무에 필요한 AI 프로그램을 만들고 프롬프트를 관리하세요
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={createProgramMutation.isPending || !title || !category}>
              {createProgramMutation.isPending ? "생성 중..." : "프로그램 만들기"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
